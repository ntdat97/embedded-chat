  ...
  useEffect(() => {
    if (siteInfo?.title) {
      if (plan !== 'basic')
        ...
        document.title = `${siteInfo.title} - Powered by Lexi`
    }
  }, [siteInfo?.title, plan])
    ...
    <div className='bg-gray-100 h-full flex flex-col rounded-2xl'>
      ...
      >
        ...
        <div className={cn(
          ...
          {
            hasSetInputs && (
              <div className={cn(doShowSuggestion ? 'pb-[140px]' : (isResponsing ? 'pb-[113px]' : 'pb-[76px]'), 'relative grow h-[200px] pc:w-[794px] max-w-full mobile:w-full mx-auto mb-3.5 overflow-hidden mt-3')}>
